package com.mm.router_ksp.auto
...
private class AutoServiceSymbolProcessor(
     ...
     * ```
     */
    private val providers: HashMap<String, HashSet<Pair<String, KSFile>>> = hashMapOf()
            ...
            .forEach { providerImplementer ->
                ...
                for (providerType in providerInterfaces) {
                    ...
                    if (checkImplementer(providerImplementer, providerType)) {
                        val set = providers[providerDecl.toBinaryName()] ?: hashSetOf()
                        set.add(providerImplementer.toBinaryName() to providerImplementer.containingFile!!)
                        providers[providerDecl.toBinaryName()] = set
                    } else {
    ...
    private fun generateAndClearConfigFiles() {
        for (providerInterface in providers.keys) {
            val resourceFile = "META-INF/services/$providerInterface"
            log("Working on resource file: $resourceFile")
            try {
                val allServices: SortedSet<String> = TreeSet()
